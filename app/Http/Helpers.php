...
{
    return Width::get();
//  return is_array($ids) ? Width::whereIn('id',$ids)->get() : '';
}

function get_temp_user_id()
{
    $temp_user_id = "";
    if (Session::has('temp_user_id')) {
        $temp_user_id = Session::get('temp_user_id');
    } else {
        $temp_user_id = rand(100, 1000) . time();
...
class Helper
{
    ...
    public static function cartCount($user_id = '')
    {
        ...
        if (Auth::check()) {
            ...
            $carts = Cart::with('product')->where('temp_user_id', $temp)->where('order_id', null)->get();
            return $carts->count();
        }
    }
    ...
    public static function totalCartPrice($user_id = '')
    {
        if (Auth::check()) {
            ...
            $temp = get_temp_user_id();
            return Cart::with('product')->where('temp_user_id', $temp)->where('order_id', null)->sum('amount');
        }
    }
