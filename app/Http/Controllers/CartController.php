...
class CartController extends Controller
{
    ...
    {
        $temp =  get_temp_user_id();
        // dd($request->all());
        $width = null;
        if ($request->width) {
            $width = Width::findOrFail($request->width);
        }
        ...
        $already_cart = Cart::where('temp_user_id',$temp)->where('order_id', null)->where('product_id', $product->id)->first();
        // return $already_cart;
        if ($already_cart) {
            // dd($already_cart);
            ...
            $cart->width = $width ? $width->title : null;
            $cart->width_id = $width ? $width->id : null;
            $cart->temp_user_id = $temp;
            $cart->product_id = $product->id;
            $cart->price = $after_discount;
            $cart->quantity = 1;
            $cart->amount =$after_discount * ($cart->quantity * ($width ? $width->title : 1));  //
    ...
    public function singleAddToCart(Request $request)
    {
        ...
        $width = null;
        if ($request->width_id) {
            $width = Width::findOrFail($request->width_id);
        }
        $product = Product::where('slug', $request->slug)->first();
        $after_discount=($product->price-(($product->price*$product->discount)/100));
        ...
        if ($already_cart) {
            ...
            $cart->amount = ($after_discount * $request->quant[1] * ($width ? $width->title : 1));
            if ($cart->product->stock < $cart->quantity || $cart->product->stock <= 0) return back()->with('error', 'Stock not sufficient!.');
        ...
        if ($request->quant) {
            ...
            foreach ($request->quant as $k => $quant) {
                ...
                if ($quant > 0 && $cart) {
                    ...
                    $cart->amount = $after_price * ($quant * ($cart->width ?? 1));
                    // return $cart->price;
                    $cart->save();
